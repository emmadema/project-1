	...
	$('.cells').click(function(){ //starts a fucntion on a click
		if (playerTurn === "player_1") { //if the player turn is player one execute the below code
			...
			if (classNames[0].attr('class') === classNames[1].attr('class')){//if the classNames match up in the array for both 1 and 2 execute code
				...
				if (points === 3){//if points are three then execute code
					...
					$(".scoreOne").append(score);//append player ones score to the game board
				}
				playerTurn = "player_2";//set the player turn to player to move on
				setTimeout(alert, 1500, 'Player 2 Turn'); //annouce that it is player twos turn only after 1.5 seconds
			...
			//clicks=0;	
			alert('Player 2 Turn');//al;ert player two that it is their turn
			}

		classNames = [];//empty the class names array
	}
		...
		if (clicks === 0) { //when the click varable is 0 execute code
			...
			if (classNames[0].attr('class') === classNames[1].attr('class')){//if the classNames match up in the array for both 1 and 2 execute code
				...
				setTimeout(alert, 1500, 'player 1 turn'); //alert player 1 turn after 1.5 seconds
		} else { //if there is no match then show the card back and hide the card front
			playerTurn = "player_1";//set the turn back to player one
			$(this).find('.cardBack').show();//show the card back
			$(this).find('.cardFront').hide();//hide the card Front
			classNames[0].hide();//hide the card front of the first card clicked
			classNames[0].parent().find('.cardBack').show();//show the card back of the first card clicked
			alert('player 1 turn');//alert player one that it is their turn
			}

		classNames = [];//clear out the class names array
		}
	}
